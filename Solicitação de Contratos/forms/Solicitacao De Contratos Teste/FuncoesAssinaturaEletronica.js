...
function inserirLinhaQuadroStatus() {
    ...
    DatasetFactory.getDataset("ds_form_aux_wesign", null, [DatasetFactory.createConstraint("numSolic", NumeroSolicitacao, NumeroSolicitacao, ConstraintType.MUST)], null, {
                ...
                if (ds.values[i].statusAssinatura == "Enviando para assinatura" || ds.values[i].statusAssinatura == "Pendente Assinatura") {
                    retorno +=
                        "<td>\
                                ...
                                <input type='hidden' class='hiddenIdCreate' value='" + ds.values[i].chaveArquivo + "'>\
...
function ativaModalAssinantes(jsonSigners) {
    ...
    for (var i = 0; i < jsonSigners.length; i++) {
		...
		<td>\
			<a>" + hex2a(jsonSigners[i].email) + "</a>\
		</td>\
		<td>\
			<a>" + hex2a(jsonSigners[i].cpf) + "</a>\
		</td>\
		<td>\
            <span class='btn " + (jsonSigners[i].status == "Assinado" ? "btn-success" : "btn-warning") + "'>" + jsonSigners[i].status + "</span>\
		</td>\
		<td>\
...
function cancelaAssinatura(id, status, cardId) {
    FLUIGC.message.confirm(
        ...
        function (result) {
            if (result) {
                deleteDocument(id, status, cardId);
...
function ModalReenviaContrato() {
	...
	<div class='col-md-6'>\
		...
		<div id='divSelecionaAssinantes'>\
			...
			<div style='display: inline-flex; width: 100%;'>\
				...
				<button class='btn btn-success btnInserirAssinantesModal' style='vertical-align: top;'>Adicionar</button>\
			</div>\
			<br><br>\
		</div>\
    ...
    myModal = FLUIGC.modal(
        ...
        function (err, data) {
            ...
            $(".btnInserirAssinantesModal").on("click", function () {
                InsereCardDeAssinanteNoPainelDeAssinantes($("#selectParteAssinaturaModal").val(), "divInfoAssinatura");
...
function CriaAssinaturaEletronica(idDoc, descricao) {
    var jsonSigners = [];
    $("#divInfoAssinatura").find(".row").each(function () {
        var listValuesAssinante = [];
        $(this).find("h3, span").each(function () {
            listValuesAssinante.push($(this).text());
        });
        jsonSigners.push({
            nome: listValuesAssinante[0],
            email: listValuesAssinante[1],
            cpf: listValuesAssinante[2],
            tipo: "E",
            status: "Pendente"
        });
    });
    ...
    var dsAux = DatasetFactory.getDataset("ds_auxiliar_wesign", null, [
        ...
        DatasetFactory.createConstraint("jsonSigners",JSON.stringify(jsonSigners), JSON.stringify(jsonSigners), ConstraintType.MUST),
        DatasetFactory.createConstraint("numSolic", $("#numProcess").val(),$("#numProcess").val(), ConstraintType.MUST)
    ], null);

    if (dsAux.values[0]["Result"] === "OK") {
        ExecutaIntegracaoAssinaturaEletronicaWeSign(idDoc);
...
function ExecutaIntegracaoAssinaturaEletronicaWeSign(codArquivo){
    var ds_upload = DatasetFactory.getDataset("ds_upload_wesign_manual", null, [
        DatasetFactory.createConstraint('codArquivo', codArquivo, codArquivo, ConstraintType.MUST)
    ], null);
    return true;
}
