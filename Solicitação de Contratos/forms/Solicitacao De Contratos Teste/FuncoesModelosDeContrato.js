function SalvarModelo() {
    if (ValidaCampos()) {
        ...
        $("#tabelaEquipamentosAditivo").find("thead").find("th").each(function(){
            headersAdtv.push($(this).find(".tableHeaderAdtv").text());
        });
        ...
        $("#tabelaEquipamentosAditivo").find("tbody").find("tr").each(function(i){
            if ($(this).hasClass("trEquipamento")) {
                ...
                $(this).find("td").each(function(j){
                    jsonEqpAdtv.VALUES.push({
                        "Chave": headersAdtv[j],
                        "Valor":  $(this).find(".campoTabelaEquipamentos").val()
                    });
                });
                jsonEquipamentosAdtv.push(jsonEqpAdtv);
            }
        });
        ...
        $("#equipamentosAditivos").val(JSON.stringify(jsonEquipamentosAdtv));
...
function EditarModelo() {
    return new Promise((resolve, reject)=>{
        ...
        $("#btnAdicionarEquipamentoAditivo").show();
...
function ValidaCampos() {
    ...
    $(".campoContrato").each(function () {
        if (($(this).val() == undefined || $(this).val() == null || $(this).val() == "") && ($("[name='radioOptAssinatura']:checked").val() != "Eletronica" || ($(this).attr("id") != "nomeFinal1" && $(this).attr("id") != "nomeFinal2" && $(this).attr("id") != "cpfFinal1" && $(this).attr("id") != "cpfFinal2"))) {
            if (($("#atividade").val() != 19 && $(this).attr("id") == "ContratoNumero") || ($("#atividade").val() != 19 && $(this).attr("id") == "CentroCusto") || $("#formMode").val() == "VIEW") {
            ...
            else if($("#idContrato").val() == 19 && (!$("#checkboxOptAcrescentarEqp").is(":checked") && $("#tabelaEquipamentos").has($(this)).length > 0 && $("#tabelaEquipamentosAditivo").has($(this)).length > 0)){
    ...
    return new Promise((res,rej) =>{
        if ($("#atividade").val() != 19) {
            ...
            $("#ContratoNumero").on("focus", function () {
                if ($(this).val() == "") {
                    $("#CentroCusto").val($("#hiddenCODGCCUSTO").val() + " - " + $("#hiddenObra").val());
        ...
        $("#btnAdicionarEquipamentoAditivo").on("click", ()=>{
            AdicionaLinhaTabelaEquipamentos("tabelaEquipamentosAditivo");
        });
        $("#btnAdicionarEquipamento").on("click", ()=>{
            AdicionaLinhaTabelaEquipamentos("tabelaEquipamentos");
        });
        $("#btnAdicionarEquipamento_1").on("click", ()=>{
            AdicionaLinhaTabelaEquipamentos("tabelaEquipamentos_1");
        });
        $("#btnRemoverEquipamento").on("click", ()=>{
            ...
            $("#tabelaRemove").find("tbody").append(tr);
...
async function gerarContrato() {
    console.log("gerarContrato");
    console.log($("#SalvaHtmlContrato").val());
    var html = $("#SalvaHtmlContrato").val();
    $("#htmlContrato").html(html);
    if($("#numProcess").val()=="645648"){
        console.log("htmlContrato ==>", html);
    }
...
function InsereValoresNosCamposAditivo() {
    return new Promise((resolve,reject) =>{
            ...
            if (i + 1 == jsonValores.length) {
                ...
                if(modelo.equipamentos == "true"){
                    ...
                    for (var i = 0; i < jsonEqp.length; i++) {
                        $("#tabelaEquipamentosAditivo").find("tbody").find("tr:nth(" + jsonEqp[i].row + ")").each(function(){
                            $(this).find("td").each(function(j){
            ...
            if (i + 1 == jsonValores.length) {
                ...
                if(modelo.equipamentos == "true"){
                    ...
                    for (var i = 0; i < jsonEqp.length; i++) {
                        ...
                        $("#tabelaEquipamentos_1").find("tbody").find("tr:nth(" + jsonEqp[i].row + ")").each(function(){
                            $(this).find("td").each(function(j){
                                $(this).find(".campoTabelaEquipamentos").val(jsonEqp[i].VALUES[j].Valor);
                                resizeInput($(this).find(".campoTabelaEquipamentos"));
                            });
                    ...
                    if($("#idModeloContrato").val() == 19) {
                                         
	                    var jsonEqpAtv = JSON.parse($("#equipamentosAditivos").val()); 
	                    for (var a = 0; a < jsonEqpAtv.length; a++) {
		                    $("#tabelaEquipamentosAditivo").find("tbody").find("tr:nth(" + jsonEqpAtv[a].row + ")").each(function(){
	                            $(this).find("td").each(function(b){
	                                $(this).find(".campoTabelaEquipamentos").val(jsonEqpAtv[a].VALUES[b].Valor);
	                                resizeInput($(this).find(".campoTabelaEquipamentos"));
	                            });
	                        });
	                    }
...
function montaPDF(){
    new Promise((resolve, reject)=>{
        ...
        for (let index = 0; index < $("#htmlContrato")[0].children.length; index++) {
            
            if($("#htmlContrato")[0].children[index].className=="printavel"){
                console.log($("#htmlContrato")[0].children[index].innerHTML)
                var elementoPrintavel = $("#htmlContrato")[0].children[index];
                if ($("input[name='radioOptAssinatura']:checked").val() == "Manual" || $(".printavel").length - 1 != index) {
                    list.push(
                        html2canvas(elementoPrintavel, { scale: 1, backgroundColor: null})   
                    );
                }
            }
        }
        ...
        resolve(Promise.all(list).then((canvas) => {
            ...
            for (var i = 0; i < vetor.length; i++) {
                ...
                var img = vetor[i].toDataURL("img/png");
                console.log("img==>",img);
            ...
            if (NMLocadora != null && NMLocadora != ""){
            	NMLocadora = NMLocadora.replace(" ", "");            	
            }
            var solicitacaoID = ($("#numProcess").val() ?? "SN").toString();
            var codigoColigada = ($("#codigoColigada").val() ?? "SN").toString();
            let retornoColigada = jsonClausulasColigadas.find((obj) => {
                return obj.coligada == codigoColigada;
            });
            ...
            var codigoNome = numero + "-" + (retornoColigada?.nomeSimplificado ?? codigoColigada) + "-";
            ...
            //console.log("codigoNome==>",codigoNome);
            //console.log("nome==>",nome);
            //console.log("fornecedor==>",fornecedor);
            nome = codigoNome + nome + "-" + fornecedor + ".pdf";
            nome = nome.split("/");
            nome = nome.join("_");
        ...
        $("#CentroCusto").val($("#spanCentroCusto").text());
        if ($("#atividade").val() == 19) {
...
function MostraOpcoesAnexosDoModelo(id){
    ...
    if (modelo.anexos.includes("FormTributacao")) {
...
function gerarWord() {
    ...
    */
    ...
    BuscaLogoColigada(codcoligada).then(function (url) {
        ...
        $("#divWord").find("#tabelaEquipamentosAditivo").attr("cellspacing", 0);
        $("#divWord").find("#tabelaEquipamentosAditivo").find("td").each(function () {
            $(this).attr("style", "border:" + $(this).css("border") + "; padding: 0px; margin:0px;");
        });
        ...
        var sourceHTML = header+document.getElementById("divWord").innerHTML+footer;
        console.log("html ==> ", sourceHTML);
        var source = 'data:application/vnd.ms-word;charset=utf-8,' + encodeURIComponent(sourceHTML);
        var fileDownload = document.createElement("a");
        document.body.appendChild(fileDownload);
        fileDownload.href = source;
        fileDownload.download = nome;
        fileDownload.click();
...
function MostraOpcoesDoModeloDeContrato(id) {
    $(".divOptLocImovel, .divOptLocEquipamento, .divOptRailLocEquipamento, .divOptTransporteMaterial, .divOptPretacaoServicos, .divOptAditivoLocEquipamentos, .divOptAditivoLocImovel, .divSelectREIDI, .divOptRescisaoLocImovel, .divOptRescisaoLocEquipamento, .divOptAditivoTransportFunc, .divOptAditivoPretacaoServicos, .divOptAditivoLocImovelPJ, .divOptAditivoLocImovelPF").hide();
                        ...
                        success: function (retorno2) {
                            ...
                            if ($("#atividade").val() == 1 || $("#atividade").val() == 0 || $("#atividade").val() == 7) {
                                $(".checkboxEqp:checked").each(function () {
                                    ...
                                    tr += "</td>\
                                        <td class='tableTd'>" + (json.CAPACIDADE ?? "") + "</td>\
                                        <td class='tableTd'>" + (json.POTENCIA ?? "") + "</td>\
                                        <td class='tableTd'>" + (json.UNIDADE ?? "") + "</td>";
                                    tr += "<td class='tableTd'>" + FormataValor(json.VALORLOCACAO) + "</td>\
                                        </tr>";
                                    $("#tbody").append(tr);
                                });
                            }
                            else {
                                var values = JSON.parse($("#objetoEquipamentos").val());
                                for (var i = 0; i < values.length; i++) {
                                    ...
                                    tr += "</td>\
                                        <td class='tableTd'>" + (values[i].CAPACIDADE ?? "") + "</td>\
                                        <td class='tableTd'>" + (values[i].POTENCIA ?? "") + "</td>\
                                        <td class='tableTd'>" + (values[i].UNIDADE ?? "") + "</td>";
                                    tr += "<td class='tableTd'>" + FormataValor(values[i].VALORLOCACAO.split(".").join("").replace(",", ".")) + "</td>\
                                        </tr>";
                                    $("#tbody").append(tr);
                                }
                            }
...
function BuscaDadosContrato() {
    ...
    //ITEM 5
    if($("#modeloContrato").val()=="Locação de Equipamento"||$("#modeloContrato").val()=="Transporte de Material"){
        if (coligada == "1") {
            representante = "Jerson";
        }
    }
    //Alteração de Representante quando Ferrovias - Locação de equipamentos
    if($("#modeloContrato").val()=="Ferrovias - Locação de Equipamento"){
        representante = "Jerson";
    }
