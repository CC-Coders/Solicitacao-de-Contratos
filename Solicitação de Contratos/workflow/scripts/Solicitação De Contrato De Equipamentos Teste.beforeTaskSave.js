...
function beforeTaskSave(colleagueId, nextSequenceId, userList) {
    ...
    //15 -> "juridico"
    //19 -> "controladoria"
    //21 -> "coord. controladoria"
    //23 -> "coord. obras"
    //30 -> "assinatura eletronica"
    //32 -> "adm. obra"
    ...
    if (hAPI.getCardValue("idDocContrato") != undefined && hAPI.getCardValue("idDocContrato") != null && hAPI.getCardValue("idDocContrato") != "") {
        ...
        for (var i = 0; i < docs.size(); i++) {
            ...
            if (doc.getDocumentId() == hAPI.getCardValue("idDocContrato")) {
                ...
                var ds = DatasetFactory.getDataset("ds_wesign_assinantes", null, [
                ...
                arrSigners.push({
                    ...
                    email: ds.getValue(0, "cEmail"),
                    cpf: ds.getValue(0, "cCpf"),
                    tipo: "E",
                    status: "Pendente",
                });
...
function cadastrarEquipamentos() {
    var containers = hAPI.getCardValue("novosContainer");
...
function atualizaStatusEquipParaEmVigencia() {
    ...
    var codigoContrato = hAPI.getCardValue("CodigoContrato").substring(0, 14);

    if (json != "" && json != null) {
        log.info(json);
        json = JSON.parse(json);
        for (var i = 0; i < json.length; i++) {
            log.info(JSON.stringify(json[i]));
            json[i].CODIGOCONTRATO = codigoContrato;
            var c1 = DatasetFactory.createConstraint("OPERACAO", "STATUSPARAVIGENCIA", "STATUSPARAVIGENCIA", ConstraintType.MUST);
            var c2 = DatasetFactory.createConstraint("JSONEQUIPAMENTO", JSONUtil.toJSON(json[i]), JSONUtil.toJSON(json[i]), ConstraintType.MUST);
            var ds = DatasetFactory.getDataset("CadastroDeEquipamentos", null, [c1, c2], null);
